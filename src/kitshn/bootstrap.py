...
def bootstrap(
    *,
    ...
    for path in (
        ...
        path.chmod(0o755)
    manifest = roots.deployments / "Caddyfile"
    if not manifest.exists():
        manifest.write_text("# Generated by KitSHn. Do not edit.\n", encoding="utf-8")
...
def _ensure_caddy_import(caddyfile: Path, roots: Roots) -> None:
    expected_import = expected_caddy_import(roots)
    if not caddyfile.exists():
        caddyfile.parent.mkdir(parents=True, exist_ok=True)
        caddyfile.write_text(expected_import + "\n", encoding="utf-8")
        return
